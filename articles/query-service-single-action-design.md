...
### QueryService自体も共有しない

これはDTOだけでなく、QueryService自体にも同じことが言えます。複数のユースケースで取得したいデータが似ているとき、既存のQueryServiceをそのまま使い回したくなりますが、今たまたま欲しい値が一致しているだけで、未来永劫一致している根拠はありません。基本的にはユースケースごとに分けた方がいいと考えています。

ただ、DTOの共有に比べると、QueryServiceの共有は実際の判断としては許容しやすい面もあります。QueryServiceの共有を後から解消するとき、影響を受けるのはそのQueryServiceを使っているユースケースだけで、それ以外には波及しません。一方DTOは、型がユースケース層を超えてハンドラーやマッパー、レスポンス変換へと広がるため、分離の影響がレイヤーをまたいで散らばります。
