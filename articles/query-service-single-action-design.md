...
「Existence」も「Count」も名詞なので、パターンは崩れません。検索ページならこうなります。

```go
type UserSearchQueryService interface {
    Execute(condition UserSearchCondition) (UserSearchDTO, error)
}
```
...
## 📝 追記：既存のQueryServiceを使い回していいのか？

この記事を公開した後、メンバーから「既存のQueryServiceで使えそうなものがあっても、共有せずにユースケース専用で作った方がいいのか？」という質問をもらいました。

DTOと同じく、基本的にはユースケースごとに分けた方がいいと考えています。今たまたま欲しい値が一致しているだけで、未来永劫一致している根拠はないからです。

ただ、DTOの共有に比べると、QueryServiceの共有は許容しやすい面もあります。QueryServiceの共有を後から解消するとき、影響を受けるのはそのQueryServiceを使っているユースケースだけで、それ以外には波及しません。一方DTOは、型がユースケース層を超えてハンドラーやマッパー、レスポンス変換へと広がるため、分離の影響がレイヤーをまたいで散らばります。

影響がユースケース内に閉じるぶん、「今は共通利用して、異なる要件が出始めたら分離する」という判断も取りやすいです。

ところで、存在確認や件数取得のように返り値がプリミティブ（`bool`や`int`）なクエリは、複数ユースケースで共有してもちろん問題ありません。DTOがないので「ユースケースごとに返す形が変わる」という前提自体が当てはまりません。
