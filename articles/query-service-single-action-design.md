...
リポジトリは `UserRepository` に `Create`・`Update`・`Delete`・`GetByID` を生やすのが一般的です。同じノリで `UserQueryService` にも複数メソッドを生やしていたのですが、プロダクトが育つにつれてじわじわ辛くなってきました。

いろいろ試した結果、自分は**シングルアクション（1インターフェース1クエリ）の方が筋が良い**と考えるようになりました。リポジトリとクエリサービスでは、責務の性質が違うからです。
